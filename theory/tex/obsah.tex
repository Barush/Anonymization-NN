\chapter{Úvod}
	...
	V souèasné dobì máme díky neustálému zlep¹ování hardwaru k dispozici obrovský výpoèetní výkon. Díky tomuto výkonu si mù¾eme troufnout i do øe¹ení tak obtí¾né úlohy jako je uèení neuronových sítí na úloze automatizované detekce a anonymizace obrazových dat. 
...
\chapter{Osobní údaje a jejich ochrana}\label{ch:personaldata}
	...
	Anonymizace dat je v dne¹ní dobì velmi aktuálním tématem. Intenzivnì se na ní pracuje jak na úrovni datové, tak na úrovni anonymizace obrazových dat, která je také pøedmìtem této práce. Obchodní spoleènost, která je schopna zajistit kvalitní anonymizaci dat, která spravuje, tím nejen plní svoje zákonné povinnosti, mù¾e si tím také vylep¹it svoje jméno v oèích zákazníkù a spoleèností, se kterými spolupracuje. 

	Tato kapitola se blí¾e zabývá legislativním podtextem ochrany osobních údajù v èásti \ref{osobni_udaje}, dále potom v èásti \ref{ch:anonymization} popisem pojmu anonymizace dat, v èásti \ref{ch:actual} jsou popsány
	metody, které se k anonymizaci dat pou¾ívají v souèastnosti a nakonec v èásti \ref{ch:data} je dokumentován postup snímání dat pro slu¾bu Panorama spoleènosti Seznam.cz. 
	...
	\section{Základy ochrany osobních údajù v ÈR}\label{osobni_udaje}
		...
		Èeské republice regulována zákonem è. 101/2001 Sb. \cite{uz_oou}, o ochranì osobních údajù a dal¹ími právními pøedpisy.
		...
		Základní ideou ochrany osobních údajù je to, ¾e osobní údaje jsou shroma¾dovány a zpracovávány za nìjakým úèelem. Ka¾dý úèel vy¾aduje jiný typ osobních údajù a také
		jiné mno¾ství takto zpracovávaných osobních údajù. Ve vý¹e uvedeném zákonì byly pro bli¾¹í specifikaci tìchto úèelù zavedeny pojmy \emph{úèel zpracování}, \emph{prostøedky 
		zpracování}, \emph{zpùsob zpracování}, \emph{kategorie osobních údajù}, \emph{kategorie subjektù údajù} a \emph{kategorie pøíjemcù}. V tìchto termínech je tøeba zpracovávání osobních údajù v¾dy pøedem popsat. \footnote{Výklad zákona 101/2001 Sb. dle http://www.oou.cz}
	...
	\section{Metody vyu¾ívané v souèasnosti}\label{ch:actual}
		...
		Anonymizace obrazových dat sestává ze dvou èástí: \emph{detekce} èásti obrázku urèené k anonymizaci a vlastního procesu \emph{anonymizace}. Je zøejmé, ¾e proces anonymizace
		...
		\subsection{Automatizovaná anonymizace}
			...
			sice na fotografii vyskytuje text, nikoliv v¹ak SPZ viz obrázek \ref{anonymized_bad}. 
			...
			\begin{figure}
				\centering
				\includegraphics[width=110mm]{fig/act_problems_cut.png}
				\caption{Problémy stávajícího øe¹ení anonymizace SPZ v Panoramì. Chybnì anonymizované nápisy nad obchody jsou na obrázku vyznaèeny èervenou barvou\protect\footnotemark.}
				\label{anonymized_bad}
			\end{figure} 		

			\footnotetext{Obrázek pochází z automatizovaného anonymizéru, který je aktuálnì pou¾íván ve spoleènosti Seznam.cz. Anonymizér byl nauèen pomocí blí¾e nespecifikovaného algoritmu strojového uèení.}	

	\section{Obrazová data pro slu¾bu Panorama}\label{ch:data}
		Obrazová data pro tuto práci byla dodána spoleèností Seznam.cz a.s. Vìt¹ina informací pro tuto èást práce byla získána v rámci nìkolika konzultací s jejími zamìstnanci, Ing. Adamem Koláøem a Ing. Miroslavem Talá¹kem. Ze spoleènosti Seznam.cz byla dále poskytnuta neveøejná prezentace o snímání obrazových dat pro slu¾bu Panorama, ze které bylo pøi pøípravì tohoto textu také vycházeno \cite{Talasek2015}. V této sekci bude popsán zpùsob, jakým jsou obrazová data pro slu¾bu Panorama snímána. Dále bude popsáno, jak jsou data dále zpracovávána, ne¾ se dostanou ke koncovému u¾ivateli. Anonymizér, který je praktickým výstupem této práce je jen jedním èlánkem v sekvenci operací, která je nad ka¾dým zachyceným obrázkem provedena. 

		\subsection{Snímání fotografií}
			Fotografie jsou snímány speciálnì upraveným vozem s 15 kamerami rozmístìnými na støe¹e. Tyto kamery zabírají 360° okolí snímacího automobilu. Zpùsob, jakým jsou kamery na støe¹e automobilu umístìny, je znázornìn na obrázku \ref{snimaci_auto}. Kamery jsou na snímací hlavì umístìny ve dvou øadách. První snímá dolní èást výsledné fotografie - v té je umístìno ¹est kamer. V druhé øadì jsou umístìny kamery snímající støední a horní èást fotografie. Na vrcholu záchytného zaøízení pro kamery je umísìna je¹tì jedna kamera míøící pøímo vzhùru. 

			\begin{figure}[h]
				\centering
				\includegraphics[width=90mm]{fig/snimaci_auto.jpg}
				\caption{Automobil s 15 kamerami umístìnými na snímací hlavì na støe¹e\protect\footnotemark.}
				\label{snimaci_auto}
			\end{figure} 
			\footnotetext{Fotografie pøevzatá z http://www.novinky.cz/internet-a-pc/348579-nova-skoda-octavia-bude-fotit-panorama-pro-mapy-cz.html}
			
			Pøi snímání se auto pohybuje rychlostí 70 km/h. Snímání se provádí na v¹ech 15 kamerách souèasnì ka¾dé 3 metry. Aby bylo mo¾né pozdìji fotografii co nejvíce pøiblí¾it tomu, jak prostøedí vnímá èlovìk, který se v nìm pohybuje, pøi ka¾dém snímku se zaznamená je¹tì sada metadat. Ta zahrnují:
			\begin{itemize}
				\item Èíslo snímku,
			...
			\begin{figure}
				\begin{minipage}{.33\linewidth}
					\centering
					\subfloat[]{\label{uhly:omega}\includegraphics[scale=.3]{fig/uhel_omega.pdf}}
				\end{minipage}%				
				\begin{minipage}{.33\linewidth}
					\centering
					\subfloat[]{\label{uhly:phi}\includegraphics[scale=.3]{fig/uhel_phi.pdf}}
				\end{minipage}%				
				\begin{minipage}{.33\linewidth}
					\centering
					\subfloat[]{\label{uhly:kappa}\includegraphics[scale=.3]{fig/uhel_kappa.pdf}}
				\end{minipage}%
				\caption{Zaznamenávané úhly pøi snímání fotomateriálu.}
				\label{uhly}
			\end{figure}

			Pro výpoèet jednotlivých úhlù jsou k dispozici speciální senzory. Pro zaznamenávání \emph{headingu} jsou na autì dvì vysoce pøesné GPS jednotky, které na základì rozdílu svých GPS poloh doká¾í pøesnì urèit smìr, kterým se snímací automobil pohybuje. Náklony ve v¹ech smìrech jsou mìøeny pomocí \emph{inerciální jednotky} a \emph{odometru}. 
		...
		\subsection{Ukládání fotografií}
			...
			Ve¹keré operace nad daty jsou provádìny v kufru automobilu, kam data ze støechy putují pøes vysokorychlostní datové kabely. Ka¾dá z 15 kamer je s poèítaèem spojena pomocí svojí vlastní gigabitové linky. 

		\subsection{Se¹ívání fotografií}
			Jakmile se data dostanou z terénu, zaèíná rozsáhlá mno¾ina operací, která je nad nimi provádìna. Jako první pøijde na øadu se¹ívání snímkù (znázornìno na obrázku \ref{sesivani}). Pomocí nástroje navr¾eného ve spoleènosti Seznam.cz je z 15 fotografií zachycených v jednom okam¾iku vytvoøena jedinná ¹irokoúhlá fotografie, nad kterou se nadále pracuje. V rámci se¹ívání fotografií jsou také odstranìny èásti snímacího automobilu, které se dostaly do zábìru.
			\begin{figure}[h]
				\centering
				\includegraphics[width=120mm]{fig/hangar.jpg}
				\caption{Se¹ívání 14 snímkù s pøekryvem do jedné panoramatické fotografie. Poslední snímek z horní kamery slou¾í k doplnìní volných míst v horní èásti fotografie.\protect\footnotemark}
				\label{sesivani}
			\end{figure} 	

		\subsection{Vyrovnávání horizontu}
			Na fotografii se¹ité z 15 fotek je znaènì deformován horizont. Nyní tedy pøichází na øadu vyrovnání horizontu zachyceného prostøedí. Pro tuto operaci byly takté¾ navr¾eny algoritmy pøímo ve spoleènosti Seznam.cz. Vzor výstupu z této operace je znázornìn na obrázku \ref{vyrovnano}. Aèkoliv obrázek nevypadá pøirozenì, po jeho namapování na kouli se ve¹keré deformace vyrovnají a vznikne vìrná kopie reálného prostøedí.
			\begin{figure}
				\centering
				\includegraphics[width=120mm]{fig/vyrovnano.jpg}
				\caption{360° fotografie po vyrovnání horizontu\protect\footnotemark.}
				\label{vyrovnano}
			\end{figure} 
			\addtocounter{footnote}{-1}
			\footnotetext{Fotografie pøevzatá z archivu firmy Seznam.cz}
			\addtocounter{footnote}{+1}
			\footnotetext{Fotografie pøevzatá z archivu firmy Seznam.cz}			

		\subsection{Zobrazení u¾ivateli}
			Výsledný obrázek je následnì namapován na kouli, na které u¾ je zobrazován koncovému u¾ivateli. Natoèení této koule v prostoru doká¾e dorovnat zakøivení, které vznikne vlivem snímání pod úhlem. V tomto okam¾iku jsou vyu¾ity zaznamenané úhly $\omega$ a $\varphi$, jejich¾ aplikace zamezí u¾ivatelskému pocitu tzv. \emph{padání obrazu}.

			\subsubsection{Ekvirektangulární projekce}
			Pro namapování obdélníkového obrazu na kouli je vyu¾ita ekvirektangulární projekce (v literatuøe té¾ oznaèována jako ekvidistantní projekce) \cite{Snyder1997}. Tato projekce vyu¾ívá pøepoèet souøadnic dle vztahù:
			 \begin{equation}
			...
			\begin{equation}
				...
				\label{ekvi_y}
			\end{equation}
			kde $x$ a $y$ jsou souøadnice na fotografii v rovinì, $\lambda$ znaèí zemìpisnou délku od centrálního poledníku v daném zobrazení, $\varphi$ znaèí zemìpisnou ¹íøku a $\varphi_1$ je parametr rovnobì¾ek, který urèuje rozsah skuteèné projekce. 		

\chapter{Neuronové sítì}\label{ch:neuronky}

	Neuronové sítì jsou jednou ze tøí hlavních podskupin vìdního oboru soft computing \cite{SekaninaSchwarz}. Dal¹ími problematikami, které tato vìdní disciplína zkoumá jsou fuzzy systémy a evoluèní algoritmy. Tato práce se vìnuje hlubokým neuronovým sítím pracujícím nad obrázkovými daty. Neuronové sítì jsou uvedeny do ¹ir¹ího kontextu v této kapitole, která je rozdìlena do nìkolika èástí. 
	...
	\section{Umìlý neuron}\label{ch:artneuron}
		...
		Jak je vidìt na obrázku \ref{art_neuron}, na vstup umìlého neuronu se pøivádí vektor èísel, ty jsou pronásobena vektorem vah, které mají zpravidla malé hodnoty (vìt¹inou v intervalu $\left<-1;1\right>$, pøípadnì $\left<-2;2\right>$), dále jsou nad nimi provedeny bázová a aktivaèní funkce (viz dále) a výstupem je jedinné èíslo. Umìlý neuron je tedy matematická struktura, která zpracovává èísla a na výstupu podává jiná èísla \cite{Kulkarni1994}. 

		\begin{description}
			\item[Bázová funkce] - urèuje zpùsob výpoètu vnitøního potenciálu $u$ neuronu. Nejèastìji vyu¾íváná je lineární bázová funkce \cite{Zboril2015}. Její vztah pro výpoèet vnitøního potenciálu je následující: 
			\begin{equation}
				u = \sum_{i = 1}^n w_i \times x_i.
				\label{base_linear}
			\end{equation} 
			Dal¹í z vyu¾ívaných bázových funkcí je radiální bázová funkce:
			\begin{equation}
			...
			\item[Aktivaèní funkce] - té¾ nazývaná \emph{pøenosová funkce}, vyu¾ívá se pro ni symbol $\sigma$. Urèuje zpùsob výpoètu výstupu neuronu z jeho vnitøního potenciálu. Ètyøi neèastìji pou¾ívané aktivaèní funkce jsou znázornìny na obrázku \ref{aktivacni_fce}. Na ose x je v¾dy hodnota vnitøního potenciálu neuronu $u$, na ose y je potom výstupní hodnota neuronu $y$.

			\begin{figure}
				\begin{minipage}{.45\linewidth}
					\centering
					\subfloat[Lineární funkce.]{\label{aktivacni:linear}\includegraphics[scale=.45]{fig/linear.pdf}}
				\end{minipage}%	
				\begin{minipage}{.45\linewidth}
					\centering
					\subfloat[Skoková funkce.]{\label{aktivacni:skokova}\includegraphics[scale=.45]{fig/skokova.pdf}}
				\end{minipage}		\\		
				\begin{minipage}{.45\linewidth}
					\centering
					\subfloat[Sigmoida.]{\label{aktivacni:sigmoida}\includegraphics[scale=.45]{fig/sigmoida.pdf}}
				\end{minipage}%				
				\begin{minipage}{.45\linewidth}
					\centering
					\subfloat[Hyperbolický tangens.]{\label{aktivacni:tanhyp}\includegraphics[scale=.45]{fig/tanhyp.pdf}}
				\end{minipage}%
	...
	\section{Vícevrstvé neuronové sítì}\label{ch:neuralnets}
		...
		\begin{description}
			...
			\item[Rozpoznávání vzorù] - na vstup je pøilo¾en vektor a na výstupu je vygenerován vzor, který nejlépe popisuje vektor poskytnutý na vstupu.
			\item[Porovnávání vzorù] - na vstup je pøilo¾en vektor znázoròující èást hledaného vzoru. Na výstup jsou potom dotvoøeny chybìjící èásti vzoru.
		...
		\subsection{Uèení neuronových sítí}\label{ch:uceni}
			...
			\begin{description}
				...
				\item[Uèení bez uèitele] je té¾ nazýváno \emph{samo-organizace}. Nebere v potaz ¾ádného vnìj¹ího uèitele a pracuje pouze s informacemi známými bìhem uèení. Algoritmy uèení bez uèitele se sna¾í zorganizovat trénovací data a nalézt v nich co nejspecifiètìj¹í spoleèné vlastnosti. 
			\end{description}

			Dále v¹ak lze uèení dìlit podle dal¹ích kritérií, jedním z nich je uèení \emph{on-line} a \emph{off-line}.

			\begin{description}
				\item[Off-line uèení] je metoda uèení, pøi které je sí» nejprve trénována na celé mno¾inì dat a a¾ v okam¾iku, kdy pro ka¾dý vstupní vektor podává oèekávané výsledky, váhy jednotlivých propojení jsou uchovány a sí» je nasazena do provozu, kde u¾ se nemìní. 

				Výhodou takto nauèené sítì je nespornì to, ¾e se chová velmi pøedvídatelnì a pro ka¾dý vstup podá korektní výstup. Nevýhodou v¹ak je, ¾e v okam¾iku, kdy chceme do mno¾iny dat pøidat dal¹í vstup, celá sí» musí být znova natrénována. Dal¹í nevýhodou tìchto metod uèení je, ¾e u¾ v okam¾iku uèení musí být k dispozici v¹echny mo¾né kombinace vstupù s jejich oèekávanými výstupy a také ¾e uèení mù¾e velmi dlouho trvat. 

				Nejznámìj¹ím zástupcem tìchto metod uèení je algoritmus \emph{backpropagation}, který je popsán dále v této kapitole.
				\item[On-line uèení] je dynamiètìj¹í metoda uèení. Pokud je do tímto zpùsobem uèené sítì potøeba pøidat dal¹í vstupní vektor, je mo¾né to udìlat za bìhu bez ztráty døíve získané informace. Výhodou sítí uèených on-line tedy je mo¾nost uèení za bìhu. 

				Pro komplikované, ale exaktnì definované problémy je dnes je¹tì výhodnìj¹í pou¾ívat metody off-line uèení. Výzvou do budoucnosti neuronových sítí v¹ak je pøijít s takovým algoritmem uèení, který by dosahoval pøesnosti off-line algoritmù, ale dokázal by pracovat s novými vstupními vektory za bìhu, tedy on-line.
			\end{description}
	...
	\section{Konvoluèní neuronové sítì}\label{ch:deeplearning}
		Konvoluèní neuronové sítì (té¾ \emph{hluboké neuronové sítì}) jsou neuronové sítì navr¾ené speciálnì pro zpracovávání obrazových dat. Na rozdíl od plnì propojených neuronových sítí, které jsme uva¾ovali v pøedchozích kapitolách, konvoluèní neuronové sítì doká¾í s výhodou vyu¾ít prostorového uspoøádání jednotlivých vstupních hodnot naèítaných z obrázku. Konvoluèní neuronové sítì se skládají ze tøí typù vrstev, které jsou podrobnì popsány dále v této kapitole: \emph{konvoluèní vrstva}, \emph{pooling vrstva} a nakonec \emph{výstupní vrstva}.

		Odli¹nost konvoluèních neuronových sítí tkví v chápání jednotlivých vrstev sítì. V plnì propojené síti je ka¾dá vrstva chápána jako lineární pole neuronù, zatímco v konvoluèní neuronové síti je mo¾né ji chápat jako dvourozmìrné pole neuronù. Toto pole svým uspoøádáním bude odpovídat uspoøádání pixelù ve zpracovávaném obrázku.

		Pro tuto kapitolu si vytvoøme jednoduchý ilustrativní obrázek o rozmìrech $10 \times 10$ pixelù, nad kterým budeme demonstrovat fungování konvoluèních neuronových sítí a zároveò provedeme srovnání poètu hledaných parametrù konvoluèní a plnì propojené sítì.

		\subsection{Vstupní vrstva}
			Vstupní vrstva konvoluèní neuronové sítì slou¾í k naètení vstupu. Stejnì jako u plnì propojených sítí, ani v konvoluèní síti vstupní vrstva neprovádí ¾ádné zpracování informace a nepoèítá se do celkového poètu vrstev. Rozmìry vstupní vrstvy neuronové sítì urèují rozmìry obrázkù, které bude mo¾né pomocí dané sítì zpracovávat, v pixelech. Vztah obrázku na vstupu a vstupní vrstvy je ilustrován na obrázku \ref{in_layer}.

		\subsection{Konvoluèní vrstva}
			Konvoluèní vrstva je stì¾ejním elementem konvoluèních neuronových sítí a zároveò první vrstvou, která nad daty ve vstupní vrstvì provádí nìjakou operaci. Konvoluèní vrstva se skládá z nìkolika map rysù (\emph{feature maps}), které pracují nezávisle na sobì.

			\subsubsection{Feature map}
				\texttt{Feature map}, tedy mapa rysù je analogií døíve popsaného \emph{vektoru rysù}. Popisuje primitiva, která se v analyzovaném vstupu nacházejí s tím rozdílem, ¾e mapa rysù ukazuje i jejich pøesnou polohu ve zdrojovém obrázku. Takovýmto primitivem tak mù¾e být napøíklad hrana v obraze. Ka¾dá mapa rysù v¹ak doká¾e obraz prohledávat se zacílením na jedinné primitivum, v reálných aplikacích je tedy tìchto map pou¾itých v¾dy více. Práci konvoluèní vrstvy v této práci uká¾eme na neuronové síti se tøemi mapami rysù, v praxi je v¹ak zpravidla vyu¾íváno map rysù nìkolikanásobnì více. 

				Ka¾dá mapa rysù pracuje nezávisle na ostatních zpùsobem, který je zobrazen na obrázku \ref{feature_map}. Vezmìme v potaz obrázek o rozmìrech $10 \times 10$\,pixelù, který u¾ máme naètený ve vstupní vrstvì a nyní má být zpracován na mapu rysù. Hodnota ka¾dého neuronu v konvoluèní vrstvì je vypoètena z hodnot podmno¾iny neuronù vstupní vrstvy - tzv. \emph{lokální pole} \texttt{local receptive fields}, nikoliv ze v¹ech jako tomu je u plnì propojených sítí. V pøípadì, který je uveden na obrázku \ref{feature_map} se jedná o podèást pole neuronù vstupní vrstvy o rozmìrech $4 \times 4$ neurony (oznaèeno modøe). Jakmile je vypoètena hodnota, pole se posouvá a je vypoètena hodnota dal¹ího neuronu z nové podmno¾iny neuronù vstupní vrstvy (v obrázku \ref{feature_map} oznaèeno rù¾ovou barvou)

				\begin{figure}[h]
					\centering
					\includegraphics[width=100mm]{fig/convolutional_layer.pdf}
					\caption{Schéma konvoluèní vrstvy sítì.}
					\label{feature_map}
				\end{figure}

				\begin{description}
					\item[Krok] - anglicky \texttt{stride} - poèet neuronù, o které se lokální pole posune mezi výpoètem hodnot dvou sousedních neuronù v mapì rysù. Základní a zároveò hojnì vyu¾ívanou hodnotou je $1$, ale bývá experimentováno i s jinými. Pøi délce kroku $1$ a velikosti lokálního pole $4 \times 4$ po vstupní vrstvì o velikosti $10 \times 10$ neuronù následuje mapa rysù o velikosti $5 \times 5$ neuronù. Pokud tedy zvolíme del¹í krok, ve výsledné síti se bude nacházet ménì neuronù, co¾ lze nìkdy s výhodou vyu¾ít. 
					\item[Sdílené váhy] - mapa rysù provádí prohledání celého vstupního obrázku na jeden konkrétní rys, proto není tøeba pro ka¾dé lokální pole odvozovat váhy jednotlivých propojení. Pro ka¾dou mapu rysù staèí pouze jedinná matice vah s jedením BIASem, která je pou¾ita pro výpoèet hodnoty ka¾dého neuronu. 
				\end{description}

			Slo¾ením nìkolika map rysù vzniká konvoluèní vrstva. Pro ilustraci v této práci budeme uva¾ovat konvoluèní vrstvu slo¾enou ze tøí map rysù, v praxi se v¹ak vyu¾ívají mnohonásobnì rozsáhlej¹í konvoluèní vrstvy. 

		\subsection{Pooling vrstva}
			\subsubsection{Max-pooling}
			\subsubsection{L2 pooling}

		\subsection{Výstupní vrstva}

		\subsection{Finetunning}


\chapter{Návrh}\label{ch:navrh}

	V této kapitole je popsán návrh implementovaného programu. Cílem této práce je vyrobit nástroj, který doká¾e automaticky zpracovávat nafocené obrázky na vstupu a podávat anonymizované obrázky na výstupu. Práce s obrázky je rozdìlena do ètyø základních fází znázornìných na obrázku \ref{overall_scheme}, jejich¾ mechanismy budou v této kapitole podrobnì popsány. Barevné hrany v obrázku \ref{overall_scheme} znázoròují jednotlivé mezivýsledky pøi zpracování a stejná barevná notace je pou¾ita i v dal¹ích obrázcích této kapitoly.  
	...
	\section{Pøedzpracování dat}\label{ch:preprocessing}	
		...
		K extrakci feature vectoru byla v této práci vyu¾ita ji¾ nauèená neuronová sí» \emph{Ilsvrc} \cite{ilsvrc_web}. Tato sí» vznikla v rámci výzvy Ilsvrc, která se koná ka¾doroènì a je zamìøena na algoritmy pro lokalizaci a detekci objektù v obraze a také na klasifikaci scény. Sí» Ilsvrc byla nauèena na mno¾inì obrázkù \emph{ImageNet}, která obsahuje více ne¾ 14 milionù obrázkù rozøazených do 1\,000 kategorií. 
		...
		\begin{figure}[h]
			\centering
			\includegraphics[width=130mm]{fig/preprocessing_nn.pdf}
	...
	\section{Detekce aut v obrázku}
		...
		\begin{figure}
			\begin{minipage}{.5\linewidth}
				\centering
				\subfloat[]{\label{rozsek:a}\includegraphics[scale=.65]{fig/rozsekani.pdf}}
			\end{minipage}%				
			\begin{minipage}{.5\linewidth}
				\centering
				\subfloat[]{\label{rozsek:b}\includegraphics[scale=.65]{fig/slozeni.pdf}}
			\end{minipage}%		
			\caption{Rozdìlení obrázku na podèásti a následné vytvoøení zátì¾ové mapy.}
			\label{rozsek+sloz}
		\end{figure}
	...
	\section{Detekce SPZ ve vybrané èásti obrázku}
		...
		\begin{description}
			\item[Detektor hran] - sada matematických metod slou¾ících k hledání výrazných (nespojitých) zmìn jasu v obrazu. Toto lze provádìt buïto na základì vy¹etøování funkce jasu a jejích derivací nebo na základì analýzy frekvencí v obraze. Mezi mnoha hranovými detektory je nejvýraznìj¹í \emph{Cannùv hranový detektor}, pøedstavený Johnem Cannym v roce 1986 \cite{Canny1986}. Tento detektor je zalo¾en na Gaussovském rozostøení obrazu. Hrany detekuje pomocí hledání orientace gradientu a smìrových derivací. Funguje velmi dobøe pøi výskytu spojitých a dostateènì silných hran. Výsledný obraz je velmi ostrý, ale ne pøíli¹ detailní.
		...
		\begin{figure}[h]
			\centering
			\includegraphics[width=130mm]{fig/plate_detection.pdf}
...
\chapter{Závìr}\label{ch:zaver}
	V rámci semestrálního projektu jsem prostudovala principy a metody vyu¾ívané pro detekci a anonymizaci SPZ vozidel zachycených na fotografiích a nástroje vyu¾ívané v souèasnosti. Dále jsem také analyzovala, které problémy souèasná øe¹ení doká¾í zpracovávat dobøe a které jsou naopak jejich slabými stránkami.
